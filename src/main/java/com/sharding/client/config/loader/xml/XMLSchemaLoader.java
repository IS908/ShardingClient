...
import com.sharding.client.config.model.rule.TableRuleConfig;
...
import com.sharding.client.util.DecryptUtil;
import com.sharding.client.util.ResourceUtil;
import com.sharding.client.util.SplitUtil;
...
public class XMLSchemaLoader implements SchemaLoader {
    ...
    private Map<String, TableConfig> loadTables(Element node, boolean isLowerCaseNames) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            TableConfig.TableTypeEnum tableType = TableConfig.TableTypeEnum.TYPE_SHARDING_TABLE;
            if ("global".equalsIgnoreCase(tableTypeStr)) {
                tableType = TableConfig.TableTypeEnum.TYPE_GLOBAL_TABLE;
            }
            //dataNode of table
    ...
    private void processChildTables(Map<String, TableConfig> tables,
        ...
        for (int j = 0; j < childNodeList.getLength(); j++) {
            ...
            TableConfig table = new TableConfig(cdTbName, primaryKey, autoIncrement, needAddLimit,
                    TableConfig.TableTypeEnum.TYPE_SHARDING_TABLE, strDatoNodes, null, false, parentTable, joinKey, parentKey);
    ...
    private DBHostConfig createDBHostConf(String dataHost, Element node, int maxCon, int minCon) {
        ...
        String dbDriver = node.getAttribute("dbDriver");
